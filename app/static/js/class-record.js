...
function updateActivityList() {
    var assessmentSelect = document.getElementById('Assessment_Score');
    var selectedAssessmentOption = assessmentSelect.options[assessmentSelect.selectedIndex];
    var selectedAssessmentId = selectedAssessmentOption.value;
    console.log("id", selectedAssessmentId);
    console.log("clicked");

    // Make an AJAX request to fetch activities based on the selected assessment
    var url = '/get_activities/' + selectedAssessmentId;
    ...
    fetch(url)
        ...
        .then(data => {
            var activityListDiv = document.querySelector('.activityList');
            activityListDiv.innerHTML = ''; // Clear previous content

            data.activities.forEach(activity => {
                var activityRow = document.createElement('div');
                activityRow.classList.add('row');
                activityRow.innerHTML = `
                    <div class="col-8">
                        <div class="form-group">
                            <h3 class="form-control">${activity[1]}</h3>
                        </div>
                    </div>
                    <div class="col-4">
                        <div class="form-group">
                            <div class="input-group">
                                <input type="text" name="scorelimit" class="form-control" placeholder="Score" maxlength="3">
                                <span class="input-group-text">/${activity[2]}</span>
                            </div>
                        </div>
                    </div>
                `;
                activityListDiv.appendChild(activityRow);
            });
